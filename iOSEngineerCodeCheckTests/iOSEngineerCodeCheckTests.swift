...
class iOSEngineerCodeCheckTests: XCTestCase {
    ...
    func testGitHubAPIFailWithZeroLengthString() throws {
        guard case .failure(let reason) = GitHubAPI.searchRepositories(for: "", completion: { _ in }) else { XCTFail(); return }
        XCTAssertEqual(reason, .searchWordTooShort)
    }

    func testRepositories() throws {
        let _ = try JSONDecoder().decode(RepoSearchResultItem.self, from: exampleJSON.data(using: .utf8)!)
    }
